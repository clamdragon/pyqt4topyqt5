...
# Simplified use from Maya: import and execute
# "pyside2converter.Main(infile, o=outfile)"
...
class PySideToPySide2(object):
    ...
    def setup(self):
        ...
        try:
            ...
            # Never seen a PySide script without indentation, but ...
            self.indent = ' '
    ...
    def get_import_lines(self, lines):
        ...
        Returns:
        (True, True, True) if there's PySide or/and QtGui or/and QtWebkit imports
        """
        ...
        def import_qwidgets():
            ...
            while i < len(lines):
                ...
                if self.is_code_line(l) and l.lstrip().startswith(('import ', 'from ')) and not '__future__' in l:
                    ...
                    lines.insert(i+1, indent + 'from PySide2.QtWidgets import *\n')
                    return
    ...
    def create_signal(self, lines, currentIdx, signal):
        """Adds the declaration of a new pysideSignal class member.
        ...
        while True:
            if self.is_code_line(line) and name in line:
                return 0
            if self.is_code_line(line) and not 'pysideSignal' in line:
                break
            currentIdx += 1
            line = lines[currentIdx]
        ...
        if len(signal) == 1 or signal[0] == 'sslErrors':
            lines.insert(currentIdx, "%s = %spysideSignal()\n" % (indent + name, module))
        else:
            type_str = ', '.join(signal[1:]).replace('::', '.')
            lines.insert(currentIdx, "%s = %spysideSignal(%s)\n" % (indent + name, module, type_str))
        self._added_pysideSignal = True
    ...
    def fix_connect(self, lines):
        """Refactor the pysideSignal.connect()

        PySide supports five versions of the connect() method:
    ...
    def fix_disconnect(self, lines):
        """Refactor the pysideSignal.disconnect()

        PySide supports three versions of the disconnect() method:
        ...
        PySide does not support these versions of the disconnect() method (but this script does):
    ...
    def fix_slot(self, lines):
        """
        pysideSignature decorator changed into pysideSlot
        clean decorator arguments
        """
        for idx, line in enumerate(lines):
            line = line.replace('@pysideSignature', '@pysideSlot')
            if '@pysideSlot' in line:
                line = self.clean_signal_args(line)
                line = line.replace("'str'", "str").replace('"str"', 'str')
            lines[idx] = line
    ...
    def fix_emit(self, lines):
        """
        Refactor the pysideSignal.emit() old-style into a new-style line.
    ...
    def fix_qchar(self, lines):
        ...
        for idx, line in enumerate(lines):
            if self.is_code_line(line):
                ...
                # use PySide2 since this method is called after change_import_lines
                line = line.replace('PySide2.QtCore.QChar', 'QChar').replace('PySide2.Qt.QChar', 'QChar')\
                           .replace('QtCore.QChar', 'QChar').replace('Qt.QChar', 'QChar')
                lines[idx] = line
                if '].connect(' in line or 'pysideSignal(' in line:
    ...
    def fix_qstring(self, lines):
        ...
        for idx, line in enumerate(lines):
            if self.is_code_line(line):
                ...
                # TODO: Convert this to use regular expressions.
                # use PySide2 since this method is called after change_import_lines
                line = line.replace('PySide2.QtCore.QString', 'QString').replace('PySide2.Qt.QString', 'QString')\
    ...
    def fix_uicShiboken(self, lines):
        """Replace calls to pysideuic and shiboken 
        with calls to pysuide2uic and shiboken2.
        ...
        for idx, line in enumerate(lines):
            if self.is_code_line(line):
                lines[idx] = line.replace('pysideuic', 'pyside2uic').replace(
                                            'shiboken', 'shiboken2')
    ...
    def remove_fromUtf8(self, lines):
        ...
        while count < len(lines):
            ...
            line = line.replace("PySide.QtCore.QString.fromUtf8(", "_fromUtf8(")\
                       .replace("PySide.Qt.QString.fromUtf8(", "_fromUtf8(")\
    ...
    def clean_signal_args(self, signal):
        ...
        signal = signal.replace(' & ', '').replace(' &', '').replace('& ', '').replace('&', '')
        signal = signal.replace("PySide_PyObject", "'PySide_PyObject'")
        if self._pyside2:
            # TODO: Convert this to use regular expressions.
            signal = signal.replace("PySide.QtCore.QString", "QString").replace("PySide.Qt.QString", "QString")\
                           .replace("QtCore.QString", "QString").replace("Qt.QString", "QString")\
                           .replace("QString", "'QString'").replace("'QString'List", "'QStringList'")\
                           .replace("'QStringList'Model", "QStringListModel")
        else:
    ...
    def change_import_lines(self, lines, backCompat=False):
        ...
        while count < len(lines):
            ...
            if line.lstrip().startswith(('import ', 'from ')):
                line = line.rstrip() + '\n'
                if self._added_pysideSignal:
                    line = self.replace_module(line, 'SIGNAL', 'pysideSignal')
                else:
            ...
            elif ls_line.startswith('from PySide.Qt import '):
            ...
            elif ls_line.startswith('from PySide.QtGui '):
                parts = line.split('import ')
            ...
            elif ls_line.startswith('from PySide.QtWebKit '):
                parts = line.split('import ')
    ...
    def save_changes(self, lines):
        with open(self.dest, 'wb') as outf:
            ...
            outf.write(''.join(lines).encode(self.tools.encoding))
...
class Main(object):
    ...
    def __init__(self, args):
        ...
        parser = argparse.ArgumentParser(description='Convert a source code '
                        'written for PySide into a valid code for PySide2')
        ...
        parser.add_argument("-o", nargs=1, help="The name of the generated "
                        ...
                        "  Default: path_PySide2 (path_PySide if --nopyside2)")
        ...
        parser.add_argument("--nolog", action="store_true",
                        ...
                        "  Default: False")
        parser.add_argument("--nopyside2", action="store_true",
                        help="Only perform updates that are compatable with PySide."
                        "  Default: False")
        ...
        if arg.nolog:
            ...
            self.log = 'pyside_to_pyside.log' if self.nopyside2 else 'pyside_to_pyside2.log'
