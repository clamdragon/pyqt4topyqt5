...
def _pyside2():
    ...
    """
    # BK
    class ExceptionHandlingApp(PySide2.QtWidgets.QApplication):
        #Overwrite notify method to force it to raise exceptions
        #to the main thread when they occur in slots.
        def notify(self, obj, event):
            try:
                return PySide2.QtWidgets.QApplication.notify(self, obj, event)
            except Exception:
                print traceback.format_exception(*sys.exc_info())

    Pyside2.QtWidgets.QApplication = ExceptionHandlingApp
    """
...
def _pyside():
    ...
    # BK
    class GuiApp(PySide.QtGui.QApplication):
        """Overwrite the translate class method to allow PySide2 style
        QApplication.translate calls to work, defaulting to Unicode"""
        @classmethod
        def translate(cls, a, b, c, e):
            par = super(GuiApp, cls)
            return par.translate(a, b, c, par.UnicodeUTF8, e)
    
    PySide.QtGui.QApplication = GuiApp
